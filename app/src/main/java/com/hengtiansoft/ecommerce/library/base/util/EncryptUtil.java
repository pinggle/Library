...
public class EncryptUtil {
    ...
    public static String encryptMd5(String input, String salt) {
        ...
        try {
            ...
            byte[] result = MessageDigest.getInstance("MD5").digest((input.substring(0, middle) + salt + input
    ...
    public static String encryptMd5(String input) {
        try {
            byte[] result = MessageDigest.getInstance("MD5").digest(input.getBytes(ENCODING));
            StringBuilder strBuilder = new StringBuilder(result.length * 2);
            for (byte b : result) {
                String s = Integer.toHexString(b & 0x00FF);
                if (1 == s.length()) {
