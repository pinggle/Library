...
public class FireView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private int startColor = Color.parseColor("#FFFF00"), endColor = Color.parseColor("#FF0000"), bottomColor = Color
            .parseColor("#70392f");
    ...
    private void drawFire(Canvas canvas) {
        ...
        if (mFirePices.size() < 10) {
            if (mFirePices.size() == 0 || mFirePices.size() > 0 && mFirePices.get(mFirePices.size() - 1).i >
                    fireWidth / 4) {
                isLeft = !isLeft;
