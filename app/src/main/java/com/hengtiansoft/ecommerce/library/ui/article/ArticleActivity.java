...
import com.hengtiansoft.ecommerce.library.view.viewholder.ArticleHeaderVH;
import com.hengtiansoft.ecommerce.library.view.viewholder.CommentItemVH;
...
public class ArticleActivity extends BaseActivity<ArticlePresenter, ArticleModel> implements ArticleContract.View {
    ...
    public void initView() {
        Image mSubject = (Image) getIntent().getSerializableExtra(Constants.HEAD_DATA);
        ...
        bt_comment.setOnClickListener(v -> {
            if (TextUtils.isEmpty(et_comment.getText().toString())) {
                ...
                mPresenter.createComment(et_comment.getText().toString(), mSubject, SharedPreferencesUtil.getUser());
        });
